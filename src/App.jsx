...
// Chamfer -- top corners on top row, bottom corners on bottom row, mid = plain rect
const CH = 14;
const CHAMFER_IDS = new Set(["sense","analyse","spark","craft","empower","lifeworlds","evidence","ideas","media","participants"]);

const chamfer = (pos, side, id) => {
  if (!id || !CHAMFER_IDS.has(id)) return "none";
  if (pos !== "top") return "none";
  const c = "25px";
  const e = "calc(100% - 25px)";
  return "polygon(" + c + " 0%, " + e + " 0%, 100% " + c + ", 100% 100%, 0% 100%, 0% " + c + ")";
};
...
function HereNowTab({ journey }) {
  ...
  const getLocation = async () => {
    ...
    try {
      ...
      } catch(e) { name = lat.toFixed(3) + ", " + lon.toFixed(3); }
    } catch(e) {
      try {
  ...
  return (
    <div style={{ marginTop:16 }}>
      {/* Single wireframe box -- always present, here and now as prompt */}
      <div
        onClick={handleTap}
        style={{
          border:"1.5px solid #ccc", padding:"20px 18px",
          minHeight: active ? "auto" : 60,
          cursor: active ? "default" : "pointer",
          animation: active ? "openCard .2s ease" : "none",
        }}>
          ...
          <div style={{ padding:"0 12px" }}>
            {selCue && selContour ? (
              <div>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:0, background:"#fff", borderTop:"1px solid #ddd8d0", borderBottom:"1px solid #ddd8d0" }}>
                  ...
                  <div style={{ background:getGroup(selContour.group).color, height:60, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <span style={{ fontSize:22, fontWeight:500, color:"#1a1a1a", letterSpacing:"-0.02em", fontFamily:"DM Sans, sans-serif",
                      borderBottom:"2.5px solid #1a1a1a", paddingBottom:1 }}>{selContour.word}</span>
                  </div>
                </div>
                <HereNowTab journey={[{ cue:selCue, contour:selContour, id:1 }]}/>
              </div>
            ) : (
              <div style={{ padding:"32px 0", textAlign:"center" }}>
                ...
                <p style={{ fontSize:10, color:"#ddd", marginTop:6, fontFamily:"Inter, sans-serif" }}>select words on the read page to begin</p>
              </div>
            )}
          </div>
        </div>
      ...
      {showInfo && (
        <div style={{ position:"fixed", inset:0, zIndex:300, background:"rgba(255,255,255,0.96)", display:"flex", flexDirection:"column", padding:"60px 28px 40px", maxWidth:390, margin:"0 auto", overflowY:"auto" }}>
          ...
          <div style={{ marginTop:32 }}>
            ...
            <p style={{ fontSize:11, color:"#aaa", fontFamily:"Inter, sans-serif", letterSpacing:"0.04em", marginBottom:16 }}>beta</p>
...
// Open card component
function OpenCard({ cue, contour, pos, group, selCue, selContour, onSelect, onClose, readOnly }) {
  const [activeType, setActiveType] = useState("cue");
  ...
  const cueSelected = selCue?.id === cue.id;
  const contourSelected = selContour?.id === contour?.id;
  const tabLeftClip  = chamfer("top", "left",  cue.id);
  const tabRightClip = chamfer("top", "right", contour?.id);
  const contentRadius = pos === "bot" ? "0 0 4px 4px" : "0";

  return (
    <div ref={cardRef} style={{ gridColumn:"1/-1", overflow:"hidden", animation:"openCard .25s ease" }}>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:G, background:GL }}>
        <button
          onClick={() => activeType === "cue" ? (!readOnly && onSelect("cue", cue)) : setActiveType("cue")}
          style={{ background:g.tint, clipPath:tabLeftClip, height:50, border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", borderBottom: activeType === "cue" ? "2px solid transparent" : "2px solid #fff", }}>
          <span style={{ fontSize:16, fontWeight:400, color:"#1a1a1a", letterSpacing:"0em", fontFamily:"DM Sans, sans-serif", borderBottom: cueSelected ? "2.5px solid #1a1a1a" : "2.5px solid transparent", paddingBottom:1 }}>{cue.word}</span>
        </button>
        {contour && (
          <button
            onClick={() => activeType === "contour" ? (!readOnly && onSelect("contour", contour)) : setActiveType("contour")}
            style={{ background:g.tint, clipPath:tabRightClip, height:50, border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", borderBottom: activeType === "contour" ? "2px solid transparent" : "2px solid #fff", }}>
            <span style={{ fontSize:16, fontWeight:400, color:"#1a1a1a", letterSpacing:"0em", fontFamily:"DM Sans, sans-serif", borderBottom: contourSelected ? "2.5px solid #1a1a1a" : "2.5px solid transparent", paddingBottom:1 }}>{contour.word}</span>
          </button>
        )}
      </div>
      <div style={{ background:g.tint }}>
        <div style={{ padding:"14px 18px 14px", display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:12 }}>
          <div style={{ flex:1 }}>
            <p style={{ fontSize:9, letterSpacing:"0.2em", textTransform:"uppercase", color:"#1a1a1a", opacity:0.5, marginBottom:6, textAlign: activeType === "contour" ? "right" : "left" }}>{activeType}</p>
            <p style={{ fontSize:14, fontStyle:"italic", color:"#555", lineHeight:1.5, fontFamily:"Georgia, serif", textAlign: activeType === "contour" ? "right" : "left" }}>{activeItem.strap}</p>
          </div>
          <button onClick={onClose} style={{ background:"none", border:"none", color:"#444", fontSize:20, cursor:"pointer", padding:0, flexShrink:0 }}>&#215;</button>
        </div>
      </div>
      <div style={{ background:g.tint, borderRadius: pos === "bot" ? "0 0 4px 4px" : "0", overflowY:"auto", maxHeight:"60vh", WebkitOverflowScrolling:"touch" }}>
        <div style={{ padding:"18px 18px 24px" }}>
          {activeItem.desc.split("\n\n").map((para, i) => (
            <p key={i} style={{ fontSize:15, fontWeight:400, color:"#1a1a1a", lineHeight:1.55, fontFamily:"Georgia, serif", marginBottom: i < activeItem.desc.split("\n\n").length - 1 ? 12 : 0, opacity:0.8 }}>{para}</p>
          ))}
        </div>
        <div style={{ position:"relative", height:180, overflow:"hidden" }}>
          <img
            src={GROUP_IMAGES[g.id]}
            alt=""
...
// Grid row
function GridRow({ cue, contour, pos, group, selCue, selContour, onSelect, openItem, onOpen, readOnly }) {
  const g = group;
  const cueSelected = selCue?.id === cue.id;
  const contourSelected = selContour?.id === contour?.id;
  const isOpen = openItem?.id === cue.id || openItem?.id === contour?.id;
  ...
  if (isOpen) {
    return (
      <OpenCard
        cue={cue} contour={contour} pos={pos} group={g}
        selCue={selCue} selContour={selContour}
        onSelect={onSelect}
        onClose={() => onOpen(null)}
        readOnly={readOnly}
      />
  ...
  return (
    <>
      <button onClick={() => onOpen(cue)} style={{ background:g.tint, clipPath:chamfer(pos,"left",cue.id), height:50, border:"none", cursor:"pointer", position:"relative", display:"flex", alignItems:"center", justifyContent:"center", transition:"opacity .1s" }}
      ...
      {contour && (
        <button onClick={() => onOpen(contour)} style={{ background:g.tint, clipPath:chamfer(pos,"right",contour?.id), height:50, border:"none", cursor:"pointer", position:"relative", display:"flex", alignItems:"center", justifyContent:"center", transition:"opacity .1s" }}
          onMouseDown={e => e.currentTarget.style.opacity=".7"} onMouseUp={e => e.currentTarget.style.opacity="1"} onMouseLeave={e => e.currentTarget.style.opacity="1"}>
          <span style={{ fontSize:16, fontWeight:400, color:"#1a1a1a", letterSpacing:"0em", fontFamily:"DM Sans, sans-serif", borderBottom: contourSelected ? "2.5px solid #1a1a1a" : "2.5px solid transparent", paddingBottom:1 }}>{contour.word}</span>
        </button>
      )}
...
// Full grid
function FullGrid({ openItem, onOpen, selCue, selContour, onSelect, readOnly }) {
  return (
    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:G, background:GL }}>
      {GROUPS.map(g => {
        const cues     = CUES.filter(c => c.group === g.id);
        ...
        return cues.map((cue, i) => {
          ...
          return (
            <GridRow key={g.id + "-" + i} cue={cue} contour={contour} pos={pos} group={g}
              selCue={selCue} selContour={selContour} onSelect={onSelect}
              openItem={rowOpen} onOpen={onOpen} readOnly={readOnly}
            />
          );
        });
      })}
...
function HereNowTab({ journey }) {
  ...
  const generateBrief = async () => {
    ...
    const place = await getLocation();
    const step  = journey[0];
    const pairNames = step.cue.word + " -- " + step.contour.word;
    const cueDesc  = step.cue.desc.split("\n\n").join(" ");
    const contDesc = step.contour.desc.split("\n\n").join(" ");
    const pairs = step.cue.word + " (cue): " + cueDesc + "\n" + step.contour.word + " (contour): " + contDesc;
    const prompt = "You are a design advisor using Design Actions, a toolkit where cues (verbs) orient design action and contours (nouns) define the terrain of inquiry.\n\nThe user is at: " + place + ".\nTheir selected pair: " + pairNames + "\n\nFull definitions:\n" + pairs + "\n\nWrite a short, evocative, place-specific text of around 150-200 words that interprets this design action pair for this specific location right now. Be concrete and grounded. Write as flowing prose, no bullet points.";
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      setAiText("Something went wrong. Tap now to retry.");
      setAiStatus("error");
    }
  };
  ...
  const copyText = () => {
    if (navigator.clipboard && aiText) navigator.clipboard.writeText(aiText);
  };

  return (
    <div style={{ marginTop:G }}>

      {/* here . now centred in their columns */}
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:G }}>
        <button onClick={() => handleTab("here")} style={{
          background:"none", border:"none", cursor:"pointer",
          padding:"12px 0", display:"flex", justifyContent:"center", width:"100%",
        }}>
          <span style={{ fontSize:16, fontWeight:400, color:"#1a1a1a", letterSpacing:"0em", fontFamily:"DM Sans, sans-serif", borderBottom: activeTab === "here" ? "2.5px solid #1a1a1a" : "2.5px solid transparent", paddingBottom:1 }}>here</span>
        </button>
        <button onClick={() => handleTab("now")} style={{
          background:"none", border:"none", cursor:"pointer",
          padding:"12px 0", display:"flex", justifyContent:"center", width:"100%",
        }}>
          <span style={{ fontSize:16, fontWeight:400, color:"#1a1a1a", letterSpacing:"0em", fontFamily:"DM Sans, sans-serif", borderBottom: activeTab === "now" ? "2.5px solid #1a1a1a" : "2.5px solid transparent", paddingBottom:1 }}>now</span>
        </button>
      </div>

      {/* Content -- generous white space */}
      {activeTab && (
        <div style={{ paddingTop:20, paddingBottom:32, paddingLeft:16, paddingRight:16, animation:"openCard .2s ease" }}>
          {activeTab === "here" && (
            <div>
              <p style={{ fontSize:9, letterSpacing:"0.2em", textTransform:"uppercase", color:"#bbb", marginBottom:10, fontFamily:"Inter, sans-serif" }}>location</p>
              {locLoading
                ? <p style={{ fontSize:15, color:"#bbb", fontFamily:"Georgia, serif" }}>locating...</p>
                : <p style={{ fontSize:15, fontWeight:400, color:"#1a1a1a", lineHeight:1.5, fontFamily:"Georgia, serif" }}>{placeName || "tap now to generate a brief"}</p>
              }
            </div>
          )}
          {activeTab === "now" && (
            <div>
              <p style={{ fontSize:9, letterSpacing:"0.2em", textTransform:"uppercase", color:"#bbb", marginBottom:10, fontFamily:"Inter, sans-serif" }}>design brief</p>
              {aiStatus === "thinking" && (
                <p style={{ fontSize:15, color:"#bbb", fontFamily:"Georgia, serif" }}>thinking...</p>
              )}
              {(aiStatus === "done" || aiStatus === "error") && (
                <div>
                  <p style={{ fontSize:15, fontWeight:400, color:"#1a1a1a", lineHeight:1.55, fontFamily:"Georgia, serif" }}>{aiText}</p>
                  <div style={{ display:"flex", gap:10, marginTop:20 }}>
                    <button onClick={generateBrief} style={{ background:"none", border:"1px solid #e0ddd8", borderRadius:4, padding:"6px 12px", fontSize:10, color:"#888", cursor:"pointer", fontFamily:"Inter, sans-serif" }}>regenerate</button>
                    <button onClick={copyText}      style={{ background:"none", border:"1px solid #e0ddd8", borderRadius:4, padding:"6px 12px", fontSize:10, color:"#888", cursor:"pointer", fontFamily:"Inter, sans-serif" }}>copy</button>
                  </div>
                </div>
              )}
            </div>
...
export default function DesignActions() {
  ...
  const setNatural = (offset) => {
    ...
    const base = idx < 0 ? 0 : idx;
    const next = NATURAL_PAIRS[(base + offset + NATURAL_PAIRS.length) % NATURAL_PAIRS.length];
    handleSelect("cue",     next.cue);
    handleSelect("contour", next.contour);
  };
    ...
    <div style={{ background:"#fff", minHeight:"100vh", color:fg, maxWidth:390, margin:"0 auto", fontFamily:"Inter, sans-serif" }}>
      <style>{`
        * { box-sizing:border-box; margin:0; padding:0; }
        button { cursor:pointer; font-family:Inter,sans-serif; }
        ::-webkit-scrollbar { display:none; }
        @keyframes openCard { from{opacity:0;transform:scaleY(0.96)} to{opacity:1;transform:scaleY(1)} }
        @keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.3} }
      `}</style>
      ...
      {screen === "home" && (
        <div style={{ paddingBottom:90 }}>
          <div style={{ padding:"52px 24px 28px" }}>
            <h1 style={{ fontSize:30, fontWeight:400, color:fg, letterSpacing:"-0.02em", lineHeight:1.0, marginBottom:10, fontFamily:"DM Sans, sans-serif" }}>Design<br/>Actions</h1>
            <p style={{ fontSize:11, fontWeight:300, color:"#888", letterSpacing:"0.12em", textTransform:"uppercase", lineHeight:1.7 }}>creative rigour<br/>for complex challenges</p>
            <p style={{ fontSize:10, fontWeight:300, color:"#888", letterSpacing:"0.08em", textTransform:"uppercase", marginTop:14 }}>tap to read . tap to select</p>
          </div>
          <div style={{ padding:"0 24px" }}>
            <FullGrid openItem={openItem} onOpen={handleOpen} selCue={selCue} selContour={selContour} onSelect={handleSelect} readOnly={false}/>
          </div>
        </div>
      )}
      ...
      {screen === "prompt" && (
        <div style={{ paddingBottom:90 }}>
          <div style={{ padding:"52px 24px 24px" }}>
            <h2 style={{ fontSize:24, fontWeight:400, color:fg, letterSpacing:"-0.01em", lineHeight:1.0, marginBottom:6, fontFamily:"DM Sans, sans-serif" }}>prompt</h2>
          </div>
          <div style={{ padding:"0 24px" }}>
            {selCue && selContour ? (
              <div>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:G, background:GL, marginBottom:0 }}>
                  <div style={{ background:getGroup(selCue.group).tint, clipPath:chamfer(getItemPos(selCue,"cue"),"left",selCue.id), height:64, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <span style={{ fontSize:17, fontWeight:400, color:"#1a1a1a", letterSpacing:"0em", fontFamily:"DM Sans, sans-serif", borderBottom:"2.5px solid #1a1a1a", paddingBottom:1 }}>{selCue.word}</span>
                  </div>
                  <div style={{ background:getGroup(selContour.group).tint, clipPath:chamfer(getItemPos(selContour,"contour"),"right",selContour.id), height:64, display:"flex", alignItems:"center", justifyContent:"center" }}>
                    <span style={{ fontSize:17, fontWeight:400, color:"#1a1a1a", letterSpacing:"0em", fontFamily:"DM Sans, sans-serif", borderBottom:"2.5px solid #1a1a1a", paddingBottom:1 }}>{selContour.word}</span>
                  </div>
                </div>
                <HereNowTab journey={[{ cue:selCue, contour:selContour, id:1 }]}/>
              </div>
              ...
              <div style={{ padding:"32px 0", textAlign:"center" }}>
                <p style={{ fontSize:12, color:"#ccc", letterSpacing:"0.06em", textTransform:"uppercase" }}>no pair selected</p>
                <p style={{ fontSize:10, color:"#ddd", marginTop:6 }}>underline words on the read page to begin</p>
              </div>
            )}
          </div>
        </div>
      ...
      {/* NAV */}
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:390, zIndex:100, background:"#fff", borderTop:"1px solid #e0ddd8", padding:"10px 24px 28px", display:"flex", alignItems:"center", justifyContent:"flex-end" }}>
        {screen === "home" ? (
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            {bothSelected && (
              <span style={{ fontSize:12, fontWeight:400, color:"#1a1a1a", letterSpacing:"0em", fontFamily:"DM Sans, sans-serif" }}>
                {selCue.word} <span style={{ color:"#ccc" }}>—</span> {selContour.word}
              </span>
            )}
            <button onClick={() => setScreen("prompt")} style={{ background: bothSelected ? "#1a1a2e" : "#e0ddd8", color: bothSelected ? "#fff" : "#aaa", border:"none", borderRadius:20, padding:"7px 18px", fontSize:11, fontWeight:500, letterSpacing:"0.04em", cursor:"pointer", transition:"background .25s" }}>
              prompt
            </button>
          </div>
        ) : (
          <div style={{ display:"flex", alignItems:"center", gap:6, flexWrap:"wrap", justifyContent:"flex-end" }}>
            {[
              { label:"+ natural", fn: () => { const n=NATURAL_PAIRS[0]; handleSelect("cue",n.cue); handleSelect("contour",n.contour); }},
              { label:"+ random",  fn: () => { handleSelect("cue",CUES[Math.floor(Math.random()*CUES.length)]); handleSelect("contour",CONTOURS[Math.floor(Math.random()*CONTOURS.length)]); }},
              { label:"back",      fn: () => setNatural(-1) },
              { label:"next",      fn: () => setNatural(1) },
            ].map(t => (
              <button key={t.label} onClick={t.fn} style={{ fontSize:10, color:"#888", border:"1px solid #e0ddd8", borderRadius:20, padding:"5px 9px", background:"none", cursor:"pointer" }}>{t.label}</button>
            ))}
            <button onClick={() => { setScreen("home"); setSelCue(null); setSelContour(null); setOpenItem(null); }} style={{ background:"#1a1a2e", color:"#fff", border:"none", borderRadius:20, padding:"7px 14px", fontSize:11, fontWeight:500, letterSpacing:"0.04em", cursor:"pointer" }}>
              read
            </button>
          </div>
        )}
